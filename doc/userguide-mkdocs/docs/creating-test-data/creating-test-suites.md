...
unless the [data-driven approach](creating-test-cases.md#data-driven-style) is used, where one test case consists of
...
cases, it is silently ignored (a message is written to the [syslog](../executing-tests/result-files.md#system-log))
...
file formats](test-data-syntax.md#supported-file-formats) (typically `__init__.robot`{.file}).
...
execution by giving multiple [paths](../executing-tests/basic-usage.md#specifying-test-data-to-be-executed).
...
related settings](creating-test-cases.md#test-case-related-settings-in-the-setting-section) is also possible. How to use different settings in the
...
!!! note
    ...
    the [--suite](../executing-tests/configuring-execution.md#by-suite-names) option to select which child suite inside it is executed.
...
be created as [test case documentation](creating-test-cases.md#test-case-name-and-documentation). For details about the syntax
...
documentation, which means that it supports [HTML formatting](../appendices/documentation-formatting.md#documentation-formatting) and [variables](variables.md#variables), and
that longer values can be [split into multiple rows](test-data-syntax.md#dividing-data-to-several-rows).
...
Not only [test cases](creating-test-cases.md#test-setup-and-teardown) but also test suites can have a setup and
