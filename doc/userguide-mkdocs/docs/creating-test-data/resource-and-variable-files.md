...
also makes them somewhat more complicated than [Variable sections](#variable-sections).
...
[reStructuredText format](#restructuredtext-format) and [JSON format](#json-format) are supported.
...
it is then searched from the directories in Python's [module search path](#module-search-path).
Searching resource files from the module search path makes it possible to
bundle them into Python packages as [package data](#package-data) and importing
...
file name](creating-user-keywords.md#using-resources-as-keyword-namespace) without the extension (for example, :name:`myresources.Some
...
Keywords created in a resource file can be [documented](creating-user-keywords.md#user-keyword-documentation) using
`[Documentation]` setting. The resource file itself can have
`Documentation` in the Setting section similarly as [suites](creating-test-suites.md#suite-documentation).
...
the same way as when parsing [reStructuredText suite files](#restructuredtext-suite-files).
...
Resource files can be converted to JSON using [ResourceFile.to_json](#resourcefile.to_json) and
recreated using [ResourceFile.from_json](#resourcefile.from_json):
...
data. Variables can also be created using [Variable sections](#variable-sections) or `set from
...
Alternatively variable files can be implemented as [classes](#classes)
...
method. Variable files can also be created as [YAML](#yaml-variable-files) and [JSON](#json-variable-files).
...
a path to the file same way as [resource files are imported](#resource-files-are-imported) using
...
in the [module search path](#module-search-path). The path can also contain variables,
...
can also be imported using the module name [similarly as libraries](#similarly-as-libraries).
When using this approach, the module needs to be in the [module search path](#module-search-path).
...
If a [variable file accepts arguments](#variable-file-accepts-arguments), they are specified after the path
...
command line are also searched from the [module search path](#module-search-path) similarly as
...
available in all test data files, similarly as [individual variables](#individual-variables)
...
All these variables can be used as a [scalar variable](#scalar-variable), lists and the
dictionary also a [list variable](#list-variable) like `@{STRINGS}` (in the dictionary's case
that variable would only contain keys), and the dictionary also as a
[dictionary variable](#dictionary-variable) like `&{MAPPING}`.
...
!!! note
    Variables are not replaced in strings got from variable files. For example,
    `VAR = "an ${example}"` would create variable `${VAR}` with a literal string
    value `an ${example}` regardless would variable `${example}` exist or not.
...
!!! note
    The `__all__` attribute is also, and originally, used by Python to decide
    which attributes to import when using the syntax `from modulename import *`.
...
!!! note
    Using YAML files with Robot Framework requires `PyYAML
    <http://pyyaml.org>`_ module to be installed. You can typically install it
    with pip_ like `pip install pyyaml`.
