...
through higher-level [user keywords](#user-keywords). This section explains how to
...
library imports similarly as with [arguments to keywords](#arguments-to-keywords).  Both the
...
keyword `Import Library` from the BuiltIn_ library. This keyword
...
implementing the library from the [module search path](#module-search-path). Libraries that
...
path has been configured, often using a custom [start-up script](#start-up-script),
...
[keyword name is prefixed with the library name](#keyword-name-is-prefixed-with-the-library-name). The library name
