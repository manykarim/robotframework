...
cases into [test suites](#test-suites) using [suite files](creating-test-suites.md#suite-files) and `suite
...
keywords. Keywords can be imported from [test libraries](#test-libraries) or `resource
files`_, or created in the [keyword section](../appendices/available-settings.md#keyword-section) of the test case file
itself.
...
is [setting variables from keyword return values](#setting-variables-from-keyword-return-values), when the second and
...
!!! note
    ...
    especially `*` is not generally recommended because they are considered to
    be [wildcards](../executing-tests/configuring-execution.md#simple-patterns) when [selecting test cases](../executing-tests/configuring-execution.md#selecting-tests). For example, trying to run
    only a test with name `Example *` like `--test 'Example *'` will
    actually run any test starting with `Example`.
...
`[Timeout]`
   Used for setting a [test case timeout](advanced-features.md#test-case-timeout). Timeouts_ are discussed in
   their own section.
...
!!! note
    ...
    deprecated in Robot Framework 3.1 and trying to use something like `[T a g
    s]` causes an error in Robot Framework 3.2. Possible spaces between brackets
    and the name (e.g. `[ Tags ]`) are still allowed.
...
`Test Timeout`
   The default value for [test case timeout](advanced-features.md#test-case-timeout). Timeouts_ are discussed in
   their own section.
...
keywords](creating-user-keywords.md#user-keyword-arguments) and [library keywords](../extending/creating-test-libraries.md#argument-types) with different arguments is
...
The test below uses keywords `Create Directory` and :name:`Copy
...
`No Operation` from BuiltIn_, takes no arguments.
...
Technically named arguments work exactly like [keyword arguments](#escaping) in Python.
...
When the named argument syntax is used with [user keywords](#user-keywords), the argument
...
If the value is a single [scalar variable](#scalar-variable), it is passed to the keyword as-is.
...
use the backslash character to [escape](test-data-syntax.md#escaping) the syntax like `foo\=quux`.
...
Naming arguments is supported by [user keywords](#user-keywords) and by most [test libraries](#test-libraries).
...
keyword arguments* or *kwargs*, similarly as [Python supports **kwargs](#[suite-setup-and-teardown](creating-test-suites.md#suite-setup-and-teardown)).
...
depends on the keyword type. For example, [Python based keywords](#python-based-keywords) simply use
`**kwargs` and [user keywords](#user-keywords) use `&{kwargs}`.
...
`Run Process` keyword in the Process_ library. It has a signature
...
As the second example, let's create a wrapper [user keyword](#user-keyword) for running the
...
not work. This syntax is inspired by Python's [keyword-only arguments](#keyword-only-arguments) syntax.
...
the [static library API](#static-library-api) [do not support this syntax](#do-not-support-this-syntax).
...
into keyword names. This syntax is supported by both [test library keywords](#test-library-keywords)
and [user keywords](#user-keywords).
...
use special [continuable failures](#continuable-failures) if stopping test execution is not desired.
...
[automatically cut from the middle](#automatically-cut-from-the-middle) to keep reports_ easier to read, but
...
they can [contain HTML formatting](#contain-html-formatting). This
...
use the [automatic variable](#automatic-variable) `${TEST_NAME}` within the test
...
If documentation gets long, it can be [split into multiple rows](#split-into-multiple-rows).
It is possible to use simple [HTML formatting](#html-formatting) and variables_ can
...
classifying test cases and also [user keywords](#user-keywords). Tags are free text and
...
- They are shown in test [reports](../executing-tests/output-files.md#report-file), [logs](../executing-tests/output-files.md#log-file) and, of course, in the test
  data, so they provide metadata to test cases.
- [Statistics](../executing-tests/output-files.md#tag-statistics) about test cases (total, passed, failed and skipped) are
  automatically collected based on them.
- They can be used to [include and exclude](../executing-tests/configuring-execution.md#by-tag-names) as well as to [skip](../executing-tests/test-execution.md#skipping-tests) test cases.
...
`Test Tags` setting in the Settings section
   ...
   If this setting is used in a [suite initialization file](#suite-initialization-file), all tests
...
to use the [escaped](test-data-syntax.md#escaping) format like `\-tag`.
...
with the `Test Tags` setting in Robot Framework 8.0 ([#5250](https://github.com/robotframework/robotframework/issues/5250)).
...
!!! note
    The `Test Tags` setting is new in Robot Framework 6.0. Earlier
    versions support `Force Tags` and `Default Tags` settings
    discussed in the next section.


!!! note
    The `-tag` syntax for removing tags using the `[Tags]` setting is
...
Tools like [Robotidy](https://robotidy.readthedocs.io) can be used to ease transition.
...
`robot:continue-on-failure` and `robot:recursive-continue-on-failure`
    Used for [enabling the continue-on-failure mode](#enabling-the-continue-on-failure-mode).

`robot:stop-on-failure` and `robot:recursive-stop-on-failure`
    Used for [disabling the continue-on-failure mode](#disabling-the-continue-on-failure-mode).

`robot:exit-on-failure`
    Stop the whole execution if a [test with this tag fails](#test-with-this-tag-fails).

`robot:skip-on-failure`
    Mark test to be [skipped if it fails](#skipped-if-it-fails).

`robot:skip`
    Mark test to be [unconditionally skipped](#unconditionally-skipped).

`robot:exclude`
    Mark test to be [unconditionally excluded](#unconditionally-excluded).
...
`robot:exit`
    Added to tests automatically when [execution is stopped gracefully](#execution-is-stopped-gracefully).
...
[tag statistics](#tag-statistics). They will be shown when they are explicitly
...
keywords using the BuiltIn_ keyword `Run Keywords`.
...
teardown are also executed even if one of them fails. This [continue on failure](#continue-on-failure)
...
test case file is using the `Test Setup` and :setting:`Test
...
!!! note
    [Test suites can have a setup and teardown of their own](#test-suites-can-have-a-setup-and-teardown-of-their-own). A suite setup
    is executed before any test cases or sub test suites in that test suite, and
    similarly a suite teardown is executed after them.
...
!!! note
    ...
    tests, but with the templated tests the mode is on automatically. If needed,
    the mode can also be [disabled](../executing-tests/test-execution.md#disabling-continue-on-failure-using-tags) with templates by using the
    `robot:stop-on-failure` tag.


!!! note
    Running all iterations if one or more is skipped is new in Robot Framework
    7.2. With earlier versions the execution stopped for the first skipped
    iteration and the test got the SKIP status regardless of the status of the
    earlier iterations.
...
the [embedded argument syntax](#embedded-argument-syntax). With templates this syntax works so
...
[IF/ELSE structures](#ifelse-structures) can be also used together with templates.
...
into the initial state (`Open Login Page` in the :name:`Valid
...
[user keyword](#user-keyword), that hides the actual test workflow. These tests are
...
!!! tip
    Naming columns like in the example above makes tests easier to understand.
    This is possible because on the header row other cells except the first one
...
corner stone of a process commonly called [Acceptance Test Driven Development](#acceptance-test-driven-development)
(ATDD) or [Specification by Example](#specification-by-example).

One way to write these requirements/tests is *Given-When-Then* style
popularized by [Behavior Driven Development](#behavior-driven-development) (BDD). When writing test cases in
...
!!! note
    These prefixes can be localized_. See the Translations_ appendix for
