...
[later in this section](#escaping), similarly as `user keyword return
values`_.
...
User keywords can have similar settings as [test cases](#test-cases), and they
...
`[Tags]`
   Sets [tags](#tags) for the keyword.
...
`[Timeout]`
   Sets the possible [user keyword timeout](advanced-features.md#user-keyword-timeout). Timeouts_ are discussed
...
!!! note
    ...
    case-insensitive and spaces are allowed between brackets and the name. For
    example, `[ TAGS ]` is valid.
...
splitting to multiple lines, and other features as [test case documentation](#test-case-documentation).
...
Both user keywords and [library keywords](#library-keywords) can have tags. Similarly as when
...
patterns, and so on, exactly as [test case tags](#test-case-tags).
...
where the keywords can also be searched based on tags. The [--removekeywords](#--removekeywords)
and [--flattenkeywords](#--flattenkeywords) commandline options also support selecting keywords by
tag, and new usages for keywords tags are possibly added in later releases.

Similarly as with [test case tags](creating-test-cases.md#tagging-test-cases), user keyword tags with the `robot:`
prefix are [reserved](creating-test-cases.md#reserved-tags) for special features by Robot Framework
...
!!! note
    versions all keyword tags need to be specified using the `[Tags]`
    setting.
...
created on [test, suite or global scope](#test,-suite-or-global-scope), but local variables of the keyword
executor cannot be used. Default value can
also be defined based on earlier arguments accepted by the keyword.


!!! note
    The syntax for default values is space sensitive. Spaces before the `=`
    sign are not allowed, and possible spaces after it are considered part of
...
support also this feature. All that is needed is having [list variable](#list-variable) such
...
any [positional argument](#positional-argument) or [named-only argument](#named-only-argument) in the keyword
signature.
...
See [free named argument examples](#free-named-argument-examples) for a full example with same keyword.
...
[variable number of arguments](#variable-number-of-arguments) (`@{varargs}`). If the keywords does not
...
Named-only arguments can be used together with [positional arguments](#positional-arguments) as
well as with [free named arguments](#free-named-arguments). When using free named arguments, they
must be last:
...
are the same, as when [creating variables](#creating-variables).
...
!!! tip
    Using `Literal`, like in the above example, is a convenient way to limit
    what values are accepted.


When using [variable number of arguments](#variable-number-of-arguments), the type is specified like
...
With [free named arguments](#free-named-arguments) the type is specified like `&{named: int}` and
...
matched possible [given/when/then/and/but prefixes](#given/when/then/and/but-prefixes) typically used in Behavior
...
they are given precedence over other keywords. Alternatively, [library search order](#library-search-order)
...
!!! note
    ...
    arguments match is a new feature in Robot Framework 6.0. With older versions
    custom regular expressions explained below can be used instead.
...
[match wrong values](#match-wrong-values) and sometimes there are conflicts that cannot
...
If needed, custom patterns can be prefixed with [inline flags](#inline-flags) such as
...
[re module](#re-module) that has pretty standard regular expressions syntax.
...
visible in [test execution errors](#test-execution-errors).
...
backslash characters [need to be escaped](#need-to-be-escaped) with another backslash, but
...
!!! note
    Prior to Robot Framework 3.2, it was mandatory to escape all closing curly
    braces in the pattern like `${digits:\d{2\}}`. This syntax is unfortunately
    not supported by Robot Framework 3.2 or newer and keywords using it must be
    updated when upgrading.


!!! note
    Prior to Robot Framework 6.0, using literal backslashes in the pattern
    required double escaping them like `${path:c:\\\\temp\\\\.*}`. Patterns
    using literal backslashes need to be updated when upgrading.
...
syntax `${name: type}` similarly as [normal user keywords](#normal-user-keywords). If a [custom pattern](#custom-pattern)
...
[omit Given, When and Then prefixes](#omit-given,-when-and-then-prefixes) in keyword definitions:
...
!!! note
    Embedded arguments feature in Robot Framework is inspired by how *step
    definitions* are created in the popular BDD tool Cucumber__.
...
using the native RETURN_ statement. The old `[Return]`
...
Regardless how values are returned, they can be [assigned to variables](#assigned-to-variables)
...
It accepts optional return values and can be used with IF_ and [inline IF](control-structures.md#inline-if)
...
!!! note
    `RETURN` syntax is case-sensitive similarly as IF_ and FOR_.


!!! note
    `RETURN` is new in Robot Framework 5.0. Use approaches explained below if
    you need to support older versions.
...
!!! note
    The `[Return]` setting was deprecated in Robot Framework 7.0 and
    the `RETURN` statement should be used instead. If there is a need to support
    older Robot Framework versions that do not support `RETURN`, it is possible
    to use the special keywords discussed in the next section.


## Using special keywords to return


BuiltIn_ keywords `Return From Keyword` and `Return From Keyword If`
...
!!! note
    These keywords are effectively deprecated and the `RETURN` statement should
    be used unless there is a need to support also older versions than Robot
    Framework 5.0. There is no visible deprecation warning when using these
    keywords yet, but they will be loudly deprecated and eventually removed in
    the future.
...
are done, the [continue on failure](#continue-on-failure) mode is enabled by default with user keyword
...
[sys.setrecursionlimit()](#sys.setrecursionlimit()) function. As the documentation of the function explains,
this should be done with care, because a too-high level can lead to a crash.


!!! note
    Prior to Robot Framework 7.2, the recursion limit was hard-coded to 100
    started keywords or control structures.
