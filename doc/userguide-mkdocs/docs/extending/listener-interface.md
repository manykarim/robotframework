...
| log_message | message | Called when an executed keyword writes a log message. `message` is a model object representing the [logged message](#resultmessage)_.<br>This method is not called if the message has level below the current [threshold level](../executing-tests/result-files.md#log-levels)_. |
