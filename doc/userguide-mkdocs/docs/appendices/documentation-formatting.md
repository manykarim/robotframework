...
It is possible to use simple HTML formatting with [test suite](../creating-test-data/test-suite.md#suite-documentation),
[test case](../creating-test-data/test-case.md#test-case-documentation) and [user keyword](../creating-test-data/creating-user-keywords.md#user-keyword-documentation) documentation and [free suite
metadata](../creating-test-data/test-suite.md#free-suite-metadata) in the test data, as well as when [documenting test
libraries](../creating-test-data/creating-test-library.md#documenting-libraries). The formatting is similar to the style used in most
...
!!! note
    ...
    `Second paragraph, this time\nwith multiple lines.` does not actually affect
    how that paragraph is rendered. Newlines are needed when creating [lists](#lists) or
    other such constructs, though.
...
between [continued documentation and metadata lines](#continued-documentation-and-metadata-lines). In practice this
...
or if it ends with an [escaping backslash](#escaping-backslash):
...
to split [list items](#list-items) to multiple rows and have [preformatted text](#preformatted-text) with spaces:
...
!!! note
    Preserving spaces in documentation and metadata is new in Robot Framework
    6.1. With earlier versions spaces need to be escaped with a backslash.
...
!!! note
    For image URLs to be recognized, the image extension is required. Data URLs
    like `data:image/png;base64,...` do not need an extension.
