...
- libraries using the [dynamic API](#varargs-library), including remote libraries,
...
## Options
  ...
  -V, --version <newversion>  Sets the version of the documented library or
                           ...
                           [defined in the source code](#kwargs-library).
  -P, --pythonpath <path>  Additional locations where to search for libraries
                           and resources similarly as when [running tests](#logginglibrary.html).
...
In the former case the library is searched using the [module search path](#module-search-path)
...
in the [module search path](#module-search-path) similarly as when executing test cases.
...
based on the example found a [bit later in this section](#bit-later-in-this-section).

![ExampleLibrary.png](ExampleLibrary.png)
...
The exact Libdoc spec file format is documented with an [XML schema](#xml-schema) (XSD)
...
!!! note
    The `XML:HTML` format introduced in Robot Framework 3.2. has been replaced
    by the format `LIBSPEC` ot the option combination `--format XML
    --specdocformat HTML`.


!!! note
    Including source information and spec version are new in Robot Framework
    3.2.
...
The exact JSON spec file format is documented with an [JSON schema](#json-schema)
at https://github.com/robotframework/robotframework/tree/master/doc/schema.
The spec file format may change between Robot Framework major releases.
...
This section discusses writing documentation for [Python](http://python.org) based test
libraries that use the static library API as well as for [dynamic libraries](#dynamic-libraries)
and [resource files](#resource-files). [Creating test libraries](../extending/creating-test-libraries.md#creating-test-libraries) and [resource files](../creating-test-data/resource-and-variable-files.md#resource-files) is
...
The documentation for Python libraries that use the [static library API](#static-library-api)
...
like can be seen [above](#general-usage), and there is also a [bit longer example](#example-library) at
...
!!! tip
    If you library does some initialization work that should not be done when
    using Libdoc, you can [easily detect is Robot Framework running](#easily-detect-is-robot-framework-running)


!!! tip
    For more information on Python documentation strings, see [PEP-257](#pep-257).
...
[implicit newline](#implicit-newline) or explicit `\n`) is considered to be the short
...
specified in [library source code](#library-source-code) using `ROBOT_LIBRARY_DOC_FORMAT`
...
formatting using `*bold*[ and ](#and)italic_`, custom links and
...
to them, the TOC also gets links to the [automatically created sections](#automatically-created-sections)
...
- URLs like http://example.com are turned to links.
- Custom links like `reStructuredText <https://docutils.sourceforge.io/rst.html>`__ are supported.
...
and argument formatting is shown also in the [longer example](#longer-example) at the
end of this chapter.
...
!!! note
    When using [reStructuredText documentation syntax](../appendices/documentation-formatting.md#restructuredtext), backticks must be
    escaped like `\\\`Keyword Name\\\``.
...
separate [importing section](#importing-section). If any of the keywords has [tags](../extending/creating-test-libraries.md#keyword-tags),
...
!!! note
    ...
    In Robot Framework 4.0 these have been removed in favor of the overview
    menu. This means that prior linking to shortcuts or tags sections does not
    work.
...
- Marker telling is the argument [positional-only](#positional-only), [named-only](#named-only),
  [free positional](#free-positional), [free named](#free-named), or [normal argument](#normal-argument) that can be given
...
the type is [automatically converted](#automatically-converted), or the type has [custom converter](#custom-converter),
...
!!! note
    Automatically listing types based on `Enum` and `TypedDict` is new in Robot
    Framework 4.0. Listing other types is new in Robot Framework 5.0.
...
on. [Click here](#click-here) to see how the generated documentation looks like.
