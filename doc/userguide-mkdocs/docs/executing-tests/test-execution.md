...
executed suite are run, but it is possible to [select tests](configuring-execution.md#selecting-tests) using
...
[continue after failures](#continue-after-failures). The exact [execution order](#execution-order) and how
...
Setups and teardowns can be used on [test suite](#test-suite), [test case](#test-case) and
[user keyword](#user-keyword) levels.
...
It is also possible to [randomize the execution order](#randomize-the-execution-order) using
...
BuiltIn_ keywords `Pass Execution` and `Pass Execution If` to stop
...
- When used in any [setup or teardown](#setup-or-teardown) (suite, test or keyword), these
...
- Possible [continuable failures](#continuable-failures) that occur before these keyword are used,
...
- It is mandatory to give an explanation message
  ...
  [documentation of these keywords](#documentation-of-these-keywords).
...
it contains fails. The keyword itself can fail by [raising an exception](#raising-an-exception) or the
...
running them at all. It works based on tags_ and supports [tag patterns](basic-usage.md#tag-patterns) like
...
Tests can also be skipped by tagging the test with the `robot:skip` [reserved tag](#reserved-tag).
...
the latter tests are [omitted from the execution altogether](#omitted-from-the-execution-altogether) and they will not
...
!!! note
    Support for using variables with tags used for skipping is new in Robot
    Framework 7.2.
...
- Using the BuiltIn_ keyword `Skip` anywhere in the test case, including setup or
...
- Using the BuiltIn_ keyword `Skip If` which takes a condition and skips the test
  if the condition is true.

- [Library keywords](#library-keywords) may also trigger skip behavior by using a special exceptions.
...
failed tests skipped. It works based on tags_ and supports [tag patterns](basic-usage.md#tag-patterns) like
...
with criticality are explained in more detail in the [issue that proposed removing it](#issue-that-proposed-removing-it).
...
`robot:recursive-stop-on-failure` tags can be used to [disable it](#disable-it).
...
[Library keywords](#library-keywords) report failures using exceptions, and it is
...
BuiltIn_ keyword `Run Keyword And Continue On Failure` allows
...
as part of `[Setup]`: The test case is marked as failed and no
test case keywords are executed.


!!! note
    The `robot:continue-on-failure` and `robot:recursive-continue-on-failure`
    tags are new in Robot Framework 4.1. They do not work properly with `WHILE`
...
[teardowns](#setups-and-teardowns) and [templates](../creating-test-data/creating-test-cases.md#test-templates):
...
alter the behavior of continuable failures caused by [library keywords](#library-keywords) or
by [Run Keyword And Continue On Failure](#run-keyword-and-continue-on-failure). For example, both keywords in this
...
!!! note
    The `robot:stop-on-failure` and `robot:recursive-stop-on-failure` tags are
    new in Robot Framework 6.0.


!!! note
    Using recursive and non-recursive tags together in same test or keyword is
...
the generated report will include `NOT robot:exit` [combined tag pattern](#combined-tag-pattern)
...
separate `Fatal Error` BuiltIn_ keyword for this purpose, and
custom keywords can use [fatal exceptions](#fatal-exceptions) when they fail.
...
If a failed test has a special `robot:exit-on-failure` [tag](../creating-test-data/creating-test-cases.md#reserved-tags), the whole execution
...
By default these errors are reported as [test execution errors](#test-execution-errors), but errors
...
!!! note
    Also logging something with the `ERROR` [log level](../extending/creating-test-libraries.md#log-levels) is considered an error
    and stops the execution if the `--exitonerror` option is used.
