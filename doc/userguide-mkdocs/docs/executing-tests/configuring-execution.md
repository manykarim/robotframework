...
outputs`_. Options related to generated [output files](output-files.md#output-files) are discussed in
the next section.
...
- `.robot` files and files that are not recognized are parsed using
  the normal [Robot Framework parser](#robot-framework-parser).
- `.rst` and `.rest` files are parsed using the [reStructuredText parser](#restructuredtext-parser).
- `.rbt` and `.json` files are parsed using the [JSON parser](#json-parser).
- Files supported by [custom parsers](#custom-parsers) are parsed by a matching parser.
...
- All files and directories starting with a dot (`.`) or an underscore
  (`_`) are ignored.
- `.robot` files are parsed using the normal [Robot Framework parser](#robot-framework-parser).
- `.robot.rst` files are parsed using the [reStructuredText parser](#restructuredtext-parser).
- `.rbt` files are parsed using the [JSON parser](#json-parser).
- Files supported by [custom parsers](#custom-parsers) are parsed by a matching parser.
...
[glob patterns](../executing-tests/configuring-execution.md#simple-patterns) like `example_*.robot`. There are, however,
...
even if they by [default would not be](#default-would-not-be). What parser to use depends on
the used extension:

- `.rst` and `.rest` files are parsed using the [reStructuredText parser](#restructuredtext-parser).
- `.json` files are parsed using the [JSON parser](#json-parser).
- Other files are parsed using the normal [Robot Framework parser](#robot-framework-parser).
...
parsed by default](../creating-test-data/test-data-syntax.md#supported-file-formats) by using the `--extension (-F)` option.
...
When [executing tasks](#executing-tasks), it is possible to use the `--task` option
...
!!! note
    Prior to Robot Framework 7.0, `--suite` with a parent suite did not
    need to match the whole suite name. For example, `parent.child` would match
    suite `child` with parent `parent` anywhere. The name must be prefixed with
    a wildcard if this behavior is desired nowadays.
...
files are parsed](#selecting-files-to-parse) if this kind of parsing optimization is needed.
...
Another way to exclude tests by tags is using the `robot:exclude` [reserved tag](#reserved-tag).
...
!!! note
    Using variables with `robot:exclude` is new in Robot Framework 7.2. Using
    variables with tags matched against `--include` and


!!! note
    In Robot Framework 7.0 `--include` and `--test` were
    cumulative and selected tests needed to match only either of these options.
    That behavior caused [backwards incompatibility problems](#backwards-incompatibility-problems) and it was
    reverted already in Robot Framework 7.0.1.
...
changed by using the `--runemptysuite` option [discussed below](#discussed-below).
...
!!! tip
    Re-execution results and original results can be [merged together](#merged-together) using
    the `--merge` command line option.
...
!!! note
    Using `--RunEmptySuite` with `--ReRunFailed` or
...
When Robot Framework parses test data, [suite names](#suite-names) are created
...
In addition to [defining documentation in the test data](#defining-documentation-in-the-test-data), documentation
of the top-level suite can be given from the command line with the
option `--doc (-D)`. The value can contain simple [HTML formatting](#html-formatting)
...
!!! note
    ...
    Prior to Robot Framework 3.1, underscores in documentation were converted
    to spaces same way as with the `--name` option.
...
`value` is its value. The value can contain simple [HTML formatting](#html-formatting) and
...
!!! note
    ...
    Prior to Robot Framework 3.1, underscores in the value were converted to
    spaces same way as with the `--name` option.
...
When Robot Framework imports a [test library](#test-library), [listener](#listener), or some other
...
a good idea to create a custom [start-up script](#start-up-script).
...
system](https://packaging.python.org/) are automatically installed so that they can be imported without
...
can also be [glob patterns](#glob-patterns) matching multiple paths, but they typically
...
!!! note
    Both colon and semicolon work regardless the operating system. Using
    semicolon is new in Robot Framework 5.0.
...
In addition to these failures, normal [execution errors](#execution-errors) are shown,
for example, when test library or resource file imports cannot be
resolved.

It is possible to disable dry run validation of specific [user keywords](#user-keywords)
by adding a special `robot:no-dry-run` [keyword tag](#keyword-tag) to them. This is useful
...
interface is explained as part of the [Robot Framework API documentation](https://robot-framework.readthedocs.io/en/latest/),
and it is possible to modify executed [test suites](https://robot-framework.readthedocs.io/en/latest/autodoc/robot.running.html#robot.running.model.TestSuite),
[test cases](https://robot-framework.readthedocs.io/en/latest/autodoc/robot.running.html#robot.running.model.TestCase) and
[keywords](https://robot-framework.readthedocs.io/en/latest/autodoc/robot.running.html#robot.running.model.Keyword) using it. The examples below ought to give an idea of
...
works exactly like when [importing a test library](#importing-a-test-library).
...
4.0, arguments also support the [named argument syntax](../creating-test-data/creating-test-cases.md#named-arguments) as well as [argument
conversion](../extending/creating-test-libraries.md#argument-conversion) based on [type hints](../extending/creating-test-libraries.md#type-hints) and [default values](../extending/creating-test-libraries.md#default-values) the same way
as keywords do.
...
results`_ can be enabled using the `--prerebotmodifier` option.
...
!!! tip
    Modifiers are taken into use from the command line exactly the same way as
    [listeners](../extending/listener-interface.md#listener-interface). See the [Registering listeners from command line](../extending/listener-interface.md#registering-listeners-from-command-line) section for
...
!!! note
    Prior to Robot Framework 4.0 `setup` and `teardown` were accessed via the
    intermediate `keywords` attribute and, for example, suite setup was disabled
    like `suite.keywords.setup = None`.
...
`dotted`
    ...
    [test execution exit](#test-execution-exit). Failed tests are listed separately
...
!!! tip
    On many UNIX-like machines you can use handy `$COLUMNS` environment
    variable like `--consolewidth $COLUMNS`.
...
using [ANSI escape codes](#ansi-escape-codes) with a backup mechanism for older Windows
...
the [support is pretty good](#support-is-pretty-good). The commonly used [Windows Console](#windows-console) does not
support links, though, but the newer [Windows Terminal](#windows-terminal) does.
...
`F` (failure) are shown on the console when using the [verbose output](#verbose-output)
